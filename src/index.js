...
client.on('messageCreate', async message => {
    ...
    // Link filtering for the whole server
    try {
        // Exempt roles (allowed to post links)
        ...
        // Allowed exact links (Magic Eden test collections)
        const ALLOWED_EXACT_URLS = [
            'https://magiceden.io/collections/monad-testnet/0x90ee6b97132420adbc6b9a8c215f0025431a53c8',
            'https://magiceden.io/collections/monad-testnet/0x209fb14943e9412354e982c4784bf89df760bf8f',
            'https://magiceden.io/collections/monad-testnet/0xfe5b5af24280af7ea190a41d0240fe30c46f1c40'
        ];

        const LOG_CHANNEL_ID = '1353880966074073109';
        const MOD_MENTIONS = EXEMPT_ROLE_IDS.map(id => `<@&${id}>`).join(' ');
        const sendErrorLog = async (text) => {
            try {
                const ch = message.client.channels.cache.get(LOG_CHANNEL_ID);
                if (ch) {
                    await ch.send({ content: `${MOD_MENTIONS} ${text}` });
                }
            } catch (err) {
                console.error('Failed to send error log message:', err);
            }
        };

        const member = await message.guild.members.fetch(message.author.id).catch(() => null);
        const isExempt = member ? EXEMPT_ROLE_IDS.some(id => member.roles.cache.has(id)) : false;
        ...
        const isAllowedUrl = (rawUrl) => {
            try {
                ...
                if (ALLOWED_EXACT_URLS.some(u => lower.startsWith(u))) return true;

                const u = new URL(rawUrl);
                const host = (u.hostname || '').toLowerCase().replace(/^www\./, '');

                // Allow X/Twitter only (not t.co)
                if (host === 'x.com' || host.endsWith('twitter.com')) return true;

                // Allow Tenor
                if (host === 'tenor.com' || host === 'media.tenor.com') return true;
        ...
        // If there are links and user is not exempt, enforce policy
        if (!isExempt && urlMatches.length > 0) {
            const hasDisallowed = urlMatches.some(url => !isAllowedUrl(url));

            if (hasDisallowed) {
                // Delete offending message
                await message.delete().catch(async err => {
                    console.error('Failed to delete message with disallowed link:', err);
                    await sendErrorLog(`⚠️ Failed to delete message with disallowed link from <@${message.author.id}> in <#${message.channel.id}>. Error: ${err?.message || err}`);
                });

                // Timeout user for 1 hour
                if (member) {
                    const ONE_HOUR_MS = 60 * 60 * 1000;
                    await member.timeout(ONE_HOUR_MS, 'Posted disallowed link').catch(async err => {
                        console.error('Failed to timeout member:', err);
                        await sendErrorLog(`⚠️ Failed to timeout <@${message.author.id}> after disallowed link. Error: ${err?.message || err}`);
                    });
                }

                // Log to the specified channel with moderator mentions
                const logChannelId = '1353880966074073109';
                const logChannel = message.client.channels.cache.get(logChannelId);
                const modMentions = EXEMPT_ROLE_IDS.map(id => `<@&${id}>`).join(' ');

                if (logChannel) {
                    const embed = new EmbedBuilder()
                        .setColor('#E74C3C')
                        .setTitle('🔗 Disallowed link removed')
                        .addFields(
                            { name: 'User', value: `${message.author.username} (${message.author.id})`, inline: false },
                            { name: 'Channel', value: `<#${message.channel.id}>`, inline: true },
                            { name: 'Timestamp', value: new Date().toISOString(), inline: true },
                            { name: 'Message Content', value: content.slice(0, 1024) || '(empty)', inline: false }
                        )
                        .setThumbnail(message.author.displayAvatarURL())
                        .setTimestamp(new Date());

                    try {
                        await logChannel.send({ content: `${modMentions}`, embeds: [embed] });
                    } catch (err) {
                        console.error('Failed to send link removal log:', err);
                        await sendErrorLog(`⚠️ Failed to send removal log for message by <@${message.author.id}>. Error: ${err?.message || err}`);
                    }
                } else {
                    console.error('Log channel not found for disallowed link');
                    await sendErrorLog('⚠️ Log channel not found for disallowed link event.');
                }

                return; // Do not award points for deleted messages
            }
        }
    } catch (e) {
        console.error('Error in link filtering:', e);
        try {
            const ch = message.client.channels.cache.get('1353880966074073109');
            if (ch) {
                const roles = ['1338993206112817283','1339293248308641883','1353403238241669132','1399886358096379964'].map(id => `<@&${id}>`).join(' ');
                await ch.send({ content: `${roles} ❗ Error in link filtering for message by <@${message.author.id}>: ${e?.message || e}` });
            }
        } catch (sendErr) {
            console.error('Failed to log link filtering error to channel:', sendErr);
        }
    }

    // Handle message points for allowed messages
