...
async function exportLeaderboards(weekly = false, archived = false) {
    try {
        ...
        try {
            ...
            // Prepare user data for Google Sheets format
            const usersValues = [
                ...
                ...userData.map(user => [
                    ...
                    user.gangName,
                    user.cash,
                    user.pointsBySource.games,
                    user.pointsBySource.memesAndArt,
                    user.pointsBySource.chatActivity,
                    user.pointsBySource.others,
                    user.pointsBySource.nftRewards
                ])
            ];

            // Prepare gang data for Google Sheets format
            const gangsValues = [
...
async function getLeaderboardData(weekly = false) {
    ...
    for (const user of users) {
        ...
        result.push({
            ...
            pointsBySource: weekly ? user.weeklyPointsBySource : user.pointsBySource
        });
    }
